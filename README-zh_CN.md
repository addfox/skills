...
独立 skill 库，供 AI 通过 [Addmo](https://github.com/addmo-build/addmo) 框架开发浏览器插件时使用。使用 [skills CLI](https://github.com/vercel-labs/skills) 通过 **`npx skills add`** 即可添加本库中的 skills。
...
| Skill | 用途 |
|-------|------|
| **migrate-to-addmo** | 从已有项目迁移到 Addmo：支持 WXT、Plasmo、Extension.js、无框架（vanilla）四种来源。 |
| **addmo-best-practices** | 使用 Addmo 开发插件时的最佳实践：入口、配置、manifest、权限、跨平台、框架与样式、消息通讯。 |
| **extension-functions-best-practices** | 实现扩展功能时的实践：视频/音频、录制、截图、AI、登录、翻译；推荐轻量库（如 Mediabunny）与优质开源扩展参考。 |
| **addmo-debugging** | 报错或需要调试时使用：优先查看 addmo/rsbuild 终端输出与 `.addmo/error.md`；用 entry、location、message、stack、front-end-framework 定位并排查；可查 `.addmo/meta.md` 确认插件基本信息。 |
| **addmo-testing** | 插件单元测试与 e2e 测试：如何配置 Rstest 的单元/e2e、适用场景、依赖安装、文件命名规范，以及按前端框架（React/Vue/Svelte/Solid 等）使用的第三方测试库。 |
